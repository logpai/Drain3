...
class TemplateMiner:
    ...
    def get_parameter_list(self, log_template: str, content: str, provide_valid_mask: Callable[[str], str] = None) -> [str]:
        ...
        try:
            template_regex = re.sub(re.escape(escaped_prefix) + r"(?!\\[*]" + re.escape(escaped_suffix) + ").+?" + re.escape(escaped_suffix),
                                    lambda x: "(" + next(masking_parameter_iterator) + ")", template_regex)
        except StopIteration:
            return []

        # If masking_parameter_iterator is not exhaused, the log_template does not match content, so we should return early.
        try:
            next(masking_parameter_iterator)
            return []
        except StopIteration:
            pass
